...
static ssize_t ece_read(struct file *fp,char *buf, size_t count, loff_t *offset)
{
    ...
    if(ret != 0)
    {
        printk(KERN_INFO "ECE3210: ERR copy to user \n");
        return -1;
    }
...
static ssize_t ece_write(struct file *fp, const char *buf, size_t count, lofft_t *op)
{
    int ret = 0;
    ...
    ece_offset_w = ece_offset_2 + count;
    return count;
}
...
int ece_init(void)
{
    ...
    {
        isReg = 0;
        printk(KERN_INFOR "ECE4310: Start FAIL \n");
    }
